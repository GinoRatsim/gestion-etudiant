...
export default function NaturePartenariat(props) {
	var datatableData = [];
	...
	login.forEach(function(item, i) {
		datatableData[i] = [item.libelleNaturePartenariat]
	});
	...
	return (
		<>
			{localStorage.getItem('id_token') === "ADMIN" ? (
				<div>
					<PageTitle title="Ajouter un type de partenariat" />
					<form onSubmit={add}>
					...
					<br />
					<PageTitle title="Liste des types de partenariat" />
